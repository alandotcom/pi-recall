...
/**
 * Agents shipped with this package, in `agents/` beside `extensions/`.
 *
 * pi's package manifest covers extensions, skills, prompts and themes, but not
 * agents, so a package that wants to ship agents has to locate them itself.
 * These rank below the user's own agents, so a same-named file in
 * ~/.pi/agent/agents wins.
 */
function findPackageAgentsDir(): string | null {
	const here = path.dirname(new URL(import.meta.url).pathname);
	const candidate = path.resolve(here, "..", "..", "agents");
...
export function discoverAgents(cwd: string, scope: AgentScope): AgentDiscoveryResult {
	...
	const packageAgentsDir = findPackageAgentsDir();

	const packageAgents = scope === "project" || !packageAgentsDir ? [] : loadAgentsFromDir(packageAgentsDir, "user");
